...
from libLF import RegExLibRegexSource as libLFRegExLibRegexSource
...
def main(regexPostsFile, outFile):
  libLFlog('Streaming patterns from regexPostsFile {} to outFile {}'.format(regexPostsFile, outFile))
  with open(regexPostsFile, 'r') as inStream, open(outFile, 'w') as outStream:
    ...
    for line in inStream:
      ...
      libLFlog('Post {} had {} patterns'.format(post.id, len(patterns)))
      ...
      if len(patterns):
        ...
        # Emit
        source.type= "StackOverflowPostRegexSource"
        outStream.write(source.toNDJSON() + '\n')

    libLFlog('Streamed {} posts (found {} pattern-like things)'.format(nPosts, nPatterns))
